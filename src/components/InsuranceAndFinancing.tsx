...
import { FINANCING as FINANCING_COPY } from "@/lib/content";
import {
  CARRIERS,
  OFFER,
  PHONE_DISPLAY,
...
/**
 * Section 9 - the affordability cards.
 *
 * Copy comes from content.ts; only the icons live here, paired by position.
 * On the model page this array held the copy too, which meant the content
 * file's FINANCING was dead - two versions of the same three cards, and the
 * one a marketer would naturally edit was the one that rendered nowhere.
 *
 * Icons are matched by index deliberately: the alternative (an icon name in
 * content.ts) puts a rendering concern in the copy file and gives a marketer
 * a string they can typo into a crash.
 */
const FINANCING_ICONS = [ShieldCheck, CreditCard, HeartHandshake];

const FINANCING = FINANCING_COPY.map((item, i) => ({
  ...item,
  icon: FINANCING_ICONS[i] ?? ShieldCheck,
}));
  ...
  return (
    <Section id="cost" className="bg-beige-light">
      ...
      <SectionHeading
        ...
        title={`ClearCorrect - ${OFFER.price} (${OFFER.savingsInline})`}
        lead={
          <>
            Your all-in ClearCorrect price is{" "}
            <strong className="font-bold text-navy">{OFFER.price}</strong> -
            that&apos;s{" "}
            <strong className="font-bold text-navy">
              {OFFER.savingsAmount} off
            </strong>{" "}
            the regular {OFFER.wasPrice}. Most PPO insurances accepted:
      ...
      */}
      <Reveal delay={0.05} className="md:hidden">
        ...
        <a
          ...
          onClick={() => trackCallClick("insurance-not-listed-grid")}
      ...
      */}
      <Reveal delay={0.12} className="hidden md:block">
        <a
          ...
          onClick={() => trackCallClick("insurance-not-listed")}
