...
import { trackBookClick, trackCallClick } from "@/lib/gtm";
...
export default function LpFooter({ year }: { year: number }) {
  ...
  return (
    <footer className="relative w-full overflow-hidden bg-navy-dark px-4 py-12 text-center sm:px-6 sm:py-16 lg:px-8">
      ...
      />
      {/* A noise-texture overlay sat here, pointing at /images/noise.webp -
          a file that has never existed in public/. It rendered as nothing at
          5% opacity and cost a 404 on every page load. Removed rather than
          re-added: the radial gradient above already does the work. */}
      ...
      <Reveal className="relative mx-auto w-full max-w-2xl">
        ...
        >
          A straighter smile starts with a free consultation.
        </h2>

        {/* max-w-xl, not max-w-md. At max-w-md each of the two columns was
            ~218px wide, and "Book My Free Consultation" needs ~205px of text
            plus an icon - so it wrapped to two lines. Widening the pair gives
            both labels a single line at every width from 320px up. */}
        <div className="mt-6 grid gap-2.5 sm:mx-auto sm:max-w-xl sm:grid-cols-2 sm:gap-3">
          <a
            href={PHONE_TEL}
            data-cta="footer-call"
            onClick={() => trackCallClick("footer")}
            className="flex items-center justify-center gap-2 whitespace-nowrap rounded-2xl bg-urgent px-3 py-3.5 text-[14px] font-bold text-white shadow-[0_12px_32px_-8px_rgba(30,96,118,0.7)] transition-colors hover:bg-urgent-dark sm:px-4 sm:text-[15px]"
          >
          ...
          <button
            type="button"
            onClick={() => {
              trackBookClick("footer");
              openBooking("footer-book");
            }}
            data-cta="footer-book"
            className="flex items-center justify-center gap-2 whitespace-nowrap rounded-2xl border border-white/25 bg-white/10 px-3 py-3.5 text-[14px] font-bold text-white backdrop-blur-md transition-colors hover:bg-white/18 sm:px-4 sm:text-[15px]"
          >
        ...
        {/* The brand bridge, per the offer lock. Many local searchers still
            know this practice by its former name. */}
