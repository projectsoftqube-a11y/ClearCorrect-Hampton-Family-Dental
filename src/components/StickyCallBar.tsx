  ...
  return (
    ...
    >
      <a
        ...
        className="flex flex-1 items-center justify-center gap-1.5 bg-navy px-2 py-4 text-[clamp(12px,3.6vw,14px)] font-bold text-white active:bg-navy-dark"
      >
        <Phone className="h-4 w-4 shrink-0" strokeWidth={2.6} aria-hidden />
        <span className="whitespace-nowrap">Call us</span>
      </a>

      {/* `book_click` fires here as well as on the dialog: the handoff maps
          that event to the Ads "Book Online Click" conversion, and the tap on
          this bar is the action the campaign is paying for - whether the
          visitor then completes the dialog is what `generate_lead` measures. */}
      <button
        type="button"
        onClick={() => {
          trackBookClick("sticky-bar");
          openBooking("sticky-book");
        }}
        data-cta="sticky-book"
        className="flex flex-1 items-center justify-center gap-1.5 bg-urgent px-2 py-4 text-[clamp(12px,3.6vw,14px)] font-bold text-white active:bg-urgent-dark"
      >
